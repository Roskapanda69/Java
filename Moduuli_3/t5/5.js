...
const kohde = document.querySelector('#pictures')

for (let i = 0; i < picArray.length; i++) {
  const kuviatahan = document.createElement('article');
        kuviatahan.className = 'card';

  const otsikko = document.createElement('h2');
        otsikko.textContent = picArray[i].title
        kuviatahan.appendChild(otsikko)

  const kehikko = document.createElement('figure')
        kehikko.caption = picArray[i].caption

  const kuvaa = document.createElement( 'img')
        kuvaa.src = picArray[i].image.medium

  kehikko.appendChild(kuvaa)
  kuviatahan.appendChild(kehikko)

  const tekstii = document.createElement('p')
        tekstii.textContent = picArray[i].description

  kuviatahan.appendChild(tekstii)

  kohde.appendChild(kuviatahan)
